...
_INTER_BASE = "https://github.com/rsms/inter/raw/master/docs/font-files"
_INTER_FILES = {
    "Inter-Bold.ttf":    f"{_INTER_BASE}/Inter-Bold.ttf",
    "Inter-Regular.ttf": f"{_INTER_BASE}/Inter-Regular.ttf",
}

def _download_fonts():
    os.makedirs(FONT_DIR, exist_ok=True)
    for name, url in _INTER_FILES.items():
        ...
        try:
            ...
            with open(dest, "wb") as f:
                f.write(r.content)
            log.info(f"Шрифт загружен: {name}")
        except Exception as e:
            log.warning(f"Не удалось загрузить шрифт {name}: {e}")
...
def _font(paths, size):
    for p in paths:
        try:
            ...
            pass
    log.warning(f"Все пути шрифтов не найдены @ {size}px — дефолт")
...
def load_fonts():
    bold = [
        ...
        "/usr/share/fonts/opentype/lato/Lato-Bold.ttf",
    ...
    return {
        "head":  _font(bold,    78),
        "sub":   _font(regular, 24),
        "tag":   _font(regular, 14),
        "hand":  _font(bold,    16),
    }
...
СТРОКА 6: пустая
СТРОКА 7: один конкретный вывод, начиная с жирного слова (без markdown)
...
"""}]
    ...
    if body_lines:
        ...
        sub = first_body[:75] + ("…" if len(first_body) > 75 else "")
...
def create_image(headline, subtext=""):
    ...
    apex = (1060, 30)
    td.polygon([apex, (700, 740), (1420, 740)], fill=(0, 200, 255, 7))
    td.line([apex, (700,  740)], fill=(0, 200, 255, 110), width=2)
    td.line([apex, (1420, 740)], fill=(0, 200, 255,  20), width=1)
    ...
    cx, cy = apex
    for r, a in [(32, 5), (20, 16), (10, 50), (5, 155)]:
        gd.ellipse([cx-r, cy-r, cx+r, cy+r], fill=(0, 220, 255, a))
    ...
    draw.line([(LM, 54), (W - LM, 54)], fill=SEP + (255,), width=1)
